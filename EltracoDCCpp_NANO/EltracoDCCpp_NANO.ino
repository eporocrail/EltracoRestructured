...
/**********************************************************************
This software is adapted to run on Arduino Nano with Arduino Nano motorshield.

The functionality has been reduced to just providing driving current.
One could say it acts like a DCC++ "booster"

The next enhancement would be to make the operation wireless via WiFi and control it via MQTT.

Maybe someone out there could port it to ESP8266/Wemos?
...
void setup() {
  ...
  Serial.println();
  Serial.println();
  Serial.print("< iDCC++ BASE STATION FOR ARDUINO ");      // Print Status to Serial Line regardless of COMM_TYPE setting so you can open Serial Monitor and check configurtion
  Serial.print(ARDUINO_TYPE);
  Serial.print(" - Motor shield: ");
  Serial.print(MOTOR_SHIELD_NAME);
  Serial.print(" >");
  Serial.println();
  Serial.print("< Firmware version: ");
  Serial.print(VERSION);
  Serial.print(" - sketch compiled on: ");
  ...
  Serial.print("< Communication via: ");
...
#define DCC_ONE_BIT_PULSE_DURATION_TIMER1 927
  ...
  // CONFIGURE TIMER_0 (NANO) TO OUTPUT 50% DUTY CYCLE DCC SIGNALS ON OC0B (UNO) INTERRUPT PINS
